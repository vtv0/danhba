...
class SuaVC12: UIViewController , UITableViewDataSource, UITableViewDelegate {
    ...
    @objc func onInputChange(_ sender: UITextField)
    {
        ...
        case 0:
            //print("0",sender.text)
            personDetails.phone[0].phoneNumber = sender.text ?? ""

        case 1:
            //print("1",sender.text)
            personDetails.phone[1].phoneNumber = sender.text ?? ""


        case 2:
            //print("2", sender.text)
            personDetails.phone[2].phoneNumber = sender.text ?? ""

        case 3:
            //print("3", sender.text)
            personDetails.phone[3].phoneNumber = sender.text ?? ""

        case 4:
            // print("4", sender.text)
            personDetails.phone[4].phoneNumber = sender.text ?? ""

        case 5:
            //print("5", sender.text)
            personDetails.phone[5].phoneNumber = sender.text ?? ""
...
//        let hiddenRecords = personDetails.phone  /*.filter({$0.DisplayStatus == false})*/
